...
var onebotOptions = []string{
	"api_url", "listen_addr", "path", "ws_path", "ping_interval",
	"secret", "access_token", "self_id",
}
...
func init() {
	bot.RegisterAdapter(bot.AdapterMetadata{
		...
		Version:     "v0.2.0",
		Author:      "core",
		Description: "OneBot v11 适配器：事件经 HTTP 上报或反向 WebSocket 进入，发送经反向 WebSocket 或 OneBot HTTP API 发出。",
...
// 反向 WebSocket 不需要出站请求，但 HTTP API 与默认客户端仍需 network 权限，
// 因此这里保持一致的失败语义。
